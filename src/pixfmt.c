...
void imlib_pixfmt_yuv422_to_rgb888(image_t *dst, image_t *src, rectangle_t *roi) {
    if (roi->x == 0 && roi->y == 0 && roi->w == dst->w && roi->h == dst->h && dst->w == src->w && dst->h == src->h) {
        ...
        for (uint32_t In = 0, Out = 0; In < src->size; In += 4, Out += 2) {
            Pixel64         = imlib_yuv442_to_rgb888(pYuv[In + 0], pYuv[In + 1], pYuv[In + 2], pYuv[In + 3]);
            pRgb24[Out + 0] = pixel32224(pPixel[1]);
            pRgb24[Out + 1] = pixel32224(pPixel[0]);
        }
    }
}
